...
//     try {
//         // Use rustfmt directly with --emit stdout to get formatted code
...
// async function createTempRustProject(code, action = "fmt") {
...
//         result = await fs.readFile(mainFilePath, "utf8");
//     } else if (action === "test") {
//         try {
...
//             };
//         } catch (error) {
//             // cargo test returns non-zero exit code on test failures
//             // but we want to capture that output rather than treat it as an error
//             result = {
//                 passed: false,
...
// async function compileSorobanContract(code) {
...
//     // Step 2: Overwrite the lib.rs file with user's code
//     const libFilePath = path.join(projectDir, "src", "lib.rs");
//     await fs.writeFile(libFilePath, code);
...
//     // Step 4: Find the generated WASM file
...
//     console.log(code);
//     try {
//         const testResults = await createTempRustProject(code, "test");
//         res.json(testResults);
//     } catch (err) {
//         console.error("Testing failed:", err);
//         res.status(500).json({ error: "Testing error." });
//     }
// });
...
//     const code = req.body;
//     try {
//         const result = await compileSorobanContract(code);
//         res.json({
//             success: true,
//             wasm: result.wasm,
...
//         `${process.env.HOME}/.cargo/bin/rustfmt --version`
//     );
//     console.log(process.env.PATH, stdout);
...
app.post("/api/projects/:projectId/compile", async (req, res) => {
    try {
        const compileResults = await compileSorobanContract(
