...
function ActivityBarButton({ children, active }) {
    const activeClass = active
        ? "text-vsdark-6 border-l-2 border-white"
        : "text-vsdark-4";
    return (
        <button
            className={`${activeClass} w-full transition min-h-12 hover:bg-neutral-500 max-h-12 flex cursor-pointer justify-center items-center outline-none`}
        >
            {children}
        </button>
    );
